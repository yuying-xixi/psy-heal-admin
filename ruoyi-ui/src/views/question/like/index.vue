<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['question:like:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['question:like:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="likeList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="点赞时间" align="center" prop="likeTime" width="180">
...
<script>
import { listLike, getLike, delLike, addLike, updateLike } from "@/api/question/like"
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除问题点赞编号为"' + ids + '"的数据项？').then(function() {
        return delLike(ids)
      }).then(() => {
        this.getList()
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('question/like/export', {
        ...this.queryParams
