    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['consult:level:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['consult:level:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listLevel, getLevel, delLevel, addLevel, updateLevel } from "@/api/consult/level"
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除咨询师等级编号为"' + ids + '"的数据项？').then(function() {
        return delLevel(ids)
      }).then(() => {
        this.getList()
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('consult/level/export', {
        ...this.queryParams
